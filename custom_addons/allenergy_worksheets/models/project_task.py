...
class ProjectTask(models.Model):
    ...
    solar_worksheet_ids = fields.One2many(
        'allenergy.worksheet.solar', 'task_id', string='Solar Worksheets')
    solar_worksheet_count = fields.Integer(
        compute='_compute_worksheet_counts')

    @api.depends('biomass_worksheet_ids', 'solar_worksheet_ids')
    def _compute_worksheet_counts(self):
        for task in self:
            task.biomass_worksheet_count = len(task.biomass_worksheet_ids)
            task.solar_worksheet_count = len(task.solar_worksheet_ids)
    ...
    def action_open_solar_worksheets(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': _('Solar Worksheets'),
            'res_model': 'allenergy.worksheet.solar',
            'view_mode': 'list,form',
            'domain': [('task_id', '=', self.id)],
            'context': {'default_task_id': self.id},
        }
